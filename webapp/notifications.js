const COMMENT_CACHE_KEY = 'ghnotif_bulk_comment_cache_v1';
        ...
        function getCommentStatus(notification) {
            ...
            if (safeIsNotificationNeedsReview(notification)) {
        ...
        function flashStatusBar() {
            elements.statusBar.classList.remove('flash');
            void elements.statusBar.offsetWidth;
            elements.statusBar.classList.add('flash');
        }

        function showStatus(message, type, { flash = false } = {}) {
        ...
        function render() {
            ...
            if (displayNotifications.length > 0) {
                displayNotifications.forEach(notif => {
                    ...
                    const commentToggleButtons = li.querySelectorAll('.comment-body-toggle-btn');
                    commentToggleButtons.forEach((toggleBtn) => {
                        toggleBtn.addEventListener('click', (e) => {
                            ...
                            if (state.commentBodyExpanded.has(commentId)) {
                                state.commentBodyExpanded.delete(commentId);
                            } else {
                                state.commentBodyExpanded.add(commentId);
                            }
                            render();
                        });
                    });
                    ...
                    updateCommentOverflowControls(li);
                });
            }
        }

        let markdownConfigured = false;
        let commentBodyMaxHeight = null;

        function getCommentBodyMaxHeight() {
            if (commentBodyMaxHeight !== null) {
                return commentBodyMaxHeight;
            }
            const value = getComputedStyle(document.documentElement)
                .getPropertyValue('--comment-body-max-height');
            const parsed = parseFloat(value);
            commentBodyMaxHeight = Number.isFinite(parsed) ? parsed : 180;
            return commentBodyMaxHeight;
        }
        ...
        function updateCommentOverflowControls(container) {
            const maxHeight = getCommentBodyMaxHeight();
            const items = container.querySelectorAll('.comment-item');
            items.forEach((item) => {
                const toggleBtn = item.querySelector('.comment-body-toggle-btn');
                const body = item.querySelector('.comment-body');
                if (!toggleBtn || !body) {
                    return;
                }
                const commentId = toggleBtn.dataset.commentId || '';
                const expanded = commentId && state.commentBodyExpanded.has(commentId);
                body.classList.toggle('collapsed', !expanded);
                const needsToggle = body.scrollHeight > maxHeight + 1;
                toggleBtn.classList.toggle('is-hidden', !needsToggle && !expanded);
